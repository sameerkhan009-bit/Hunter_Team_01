...
let allLamixRanges = [];
let allLamixNumbers = [];
let allLamixMessages = [];
let copiedNumbersMap = JSON.parse(localStorage.getItem('ht_copied_direct')) || {};
...
// 4. Load 100% Direct Data from Lamix API
async function loadDirectLamixData() {
    try {
        ...
        allLamixRanges = Array.isArray(rangesRes) ? rangesRes : [];
        allLamixNumbers = Array.isArray(numbersRes) ? numbersRes : [];
        allLamixMessages = Array.isArray(messagesRes) ? messagesRes : [];

        // Build Linked Ranges
        renderLiveRanges();
        renderLiveInbox();

        // Update Live SMS Counter directly from Lamix
        const liveCount = allLamixMessages.length;
        document.getElementById('todaySmsVal').textContent = liveCount;
        document.getElementById('inboxTodayVal').textContent = liveCount;
        document.getElementById('inboxWeekVal').textContent = liveCount;
        document.getElementById('inboxMonthVal').textContent = liveCount;
        document.getElementById('navInboxCount').textContent = liveCount;
        document.getElementById('msgCountBadge').textContent = liveCount;
    } catch (e) {
        console.error('Error loading Lamix data:', e);
    }
}

// 5. Render Real Ranges & Map Real Numbers
function renderLiveRanges(list = allLamixRanges) {
    ...
    container.innerHTML = list.map((r, idx) => {
        ...
        // Match real numbers belonging to this range
        const matchedNumbers = allLamixNumbers.filter(n => {
        ...
        const count = matchedNumbers.length || r.count || r.numbers_count || 0;

        return `
            <div class="range-item-row" onclick="openLiveRangeDetail('${r.id || idx}', '${encodeURIComponent(rName)}', '${rPrefix}')">
                <div class="item-left">
                    <div class="sat-ico">📡</div>
                    <div>
                        <div class="range-name-bold">${rName}</div>
                        <div class="range-active-sub">${count} numbers · ● Active</div>
                    </div>
                </div>
...
function filterRangesList() {
    ...
    const filtered = allLamixRanges.filter(r => (r.name || r.range_name || '').toLowerCase().includes(q));
    renderLiveRanges(filtered);
}

// 6. Open Range and Show Real Lamix Numbers
function openLiveRangeDetail(id, encodedName, prefix) {
    const rName = decodeURIComponent(encodedName);

    // Filter all real numbers for this range
    const matchedNumbers = allLamixNumbers.filter(n => {
        const numRange = n.range || n.range_name || '';
        const rawNum = String(n.number || n.phone || n);
        return numRange === rName || (prefix && rawNum.startsWith(prefix));
    }).map(n => String(n.number || n.phone || n));
    ...
    document.getElementById('rangeDetailHeading').textContent = `📡 ${rName}`;

    renderLiveRangeNumbers();
}
...
function renderLiveRangeNumbers() {
    ...
    if (numbers.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:35px; color:#8c9bb5;">No numbers found in Lamix for this range yet.</div>`;
        return;
    }
...
function copyNumberString(num) {
    ...
    localStorage.setItem('ht_copied_direct', JSON.stringify(copiedNumbersMap));
    renderLiveRangeNumbers();
    showToast("Copied");
}
...
function resetRangeCopied() {
    if (confirm("Reset copied numbers status?")) {
        copiedNumbersMap = {};
        localStorage.removeItem('ht_copied_direct');
        renderLiveRangeNumbers();
        showToast("Reset Done");
    }
}

// 7. Render Direct Real Messages from Lamix
function renderLiveInbox() {
    const container = document.getElementById('inboxFeedContainer');

    if (allLamixMessages.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:35px; color:#8c9bb5;">No messages received in Lamix yet.</div>`;
        return;
    }

    container.innerHTML = allLamixMessages.map(m => {
...
checkAppSession();
renderTop10();
setInterval(loadDirectLamixData, 8000); // Live poll Lamix API every 8 seconds
