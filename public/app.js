...
let copiedNumbersMap = JSON.parse(localStorage.getItem('ht_copied_nums_live')) || {};
...
function appLogin() {
    ...
    // Client Verification via Lamix Live API
...
function fetchLiveLamixData() {
    ...
    fetch(`/api/live-data?user=${encodeURIComponent(currentUser)}`)
        ...
        .then(data => {
            if (data.success) {
                // Set Real Ranges from Lamix
                liveRangesList = data.ranges || [];
                renderRanges(liveRangesList);

                // Set Real Messages & SMS from Lamix
                liveMessagesList = data.messages || [];
                renderInboxFeed(liveMessagesList);

                // Set Real SMS Counter
                const todayTotal = data.todaySms || liveMessagesList.length || 0;
                document.getElementById('todaySmsVal').textContent = todayTotal;
                document.getElementById('inboxTodayVal').textContent = todayTotal;
                document.getElementById('navInboxCount').textContent = liveMessagesList.length;
            }
        })
        .catch(() => {});
}
...
function renderRanges(list = liveRangesList) {
    ...
    if (list.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:35px; color:#8c9bb5;">No ranges assigned in Lamix for this user.</div>`;
        return;
    }
        ...
        <div class="range-item-row" onclick="openRangeDetail('${r.id}')">
            <div class="item-left">
                ...
                <div>
                    ...
                    <div class="range-active-sub">${r.count || r.numbers.length} numbers · ● Active</div>
                </div>
            </div>
...
function copyNumberString(num) {
    ...
    localStorage.setItem('ht_copied_nums_live', JSON.stringify(copiedNumbersMap));
    renderRangeNumbers();
    showToast("Copied");
}
...
function resetRangeCopied() {
    if (confirm("Reset copied numbers status?")) {
        copiedNumbersMap = {};
        localStorage.removeItem('ht_copied_nums_live');
        renderRangeNumbers();
...
checkAppSession();
renderTop10();
setInterval(fetchLiveLamixData, 10000); // Live poll Lamix API every 10 seconds
