...
let copiedNumbersMap = JSON.parse(localStorage.getItem('ht_copied_nums_live_v8')) || {};
    ...
    fetch(`/api/live-data?user=${encodeURIComponent(currentUser)}`)
        ...
        .then(data => {
            if (data.success && data.ranges && data.ranges.length > 0) {
                ...
                const totalSms = data.todaySms || liveMessagesList.length || 957;
                document.getElementById('todaySmsVal').textContent = totalSms;
                document.getElementById('inboxTodayVal').textContent = totalSms;
                document.getElementById('navInboxCount').textContent = liveMessagesList.length || 3;
            }
        })
        .catch(() => {});
}
...
function copyNumberString(num) {
    ...
    localStorage.setItem('ht_copied_nums_live_v8', JSON.stringify(copiedNumbersMap));
    renderRangeNumbers();
    showToast("Copied");
}
...
function resetRangeCopied() {
    if (confirm("Reset copied numbers status?")) {
        copiedNumbersMap = {};
        localStorage.removeItem('ht_copied_nums_live_v8');
        renderRangeNumbers();
...
function renderInboxFeed(messages = liveMessagesList) {
    ...
    const displayList = (messages && messages.length > 0) ? messages : fallbackDemoInbox;
    ...
    container.innerHTML = displayList.map(m => {
        const sender = m.sender || m.from || m.phone || '+447873077777';
...
checkAppSession();
renderTop10();
setInterval(fetchLiveLamixData, 8000); // Live poll Lamix API every 8 seconds
