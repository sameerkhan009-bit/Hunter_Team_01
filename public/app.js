...
let copiedNumbersMap = JSON.parse(localStorage.getItem('ht_copied_highscale')) || {};
...
// 3. User Login (Admin & All 100+ Clients)
function appLogin() {
...
// 4. Load Live Ranges & Messages from Lamix
async function loadLivePortalData() {
    try {
        ...
        if (Array.isArray(msgsRes)) {
            ...
            document.getElementById('navInboxCount').textContent = liveMessagesList.length || 0;
            document.getElementById('msgCountBadge').textContent = liveMessagesList.length || 0;
        }
    } catch (e) {}
}

// 5. Render All Live Ranges Dynamically
...
// 6. On-Demand Fetching for 100,000+ Numbers (Instant & Zero Lag)
async function openRangeNumbersLive(id, encodedName, prefix) {
    const name = decodeURIComponent(encodedName);
    currentSelectedRange = { id, name, prefix, numbers: [] };
    ...
    container.innerHTML = `<div style="text-align:center; padding:35px; color:#8c9bb5;">Loading numbers from Lamix...</div>`;

    try {
        const res = await fetch(`/api/range-numbers?range=${encodeURIComponent(name)}&prefix=${prefix}&id=${id}`);
        const numbers = await res.json();
        currentSelectedRange.numbers = Array.isArray(numbers) ? numbers : [];
        renderRangeNumbers();
    } catch (e) {
        container.innerHTML = `<div style="text-align:center; padding:35px; color:#ef4444;">Failed to load numbers for this range.</div>`;
    }
}
...
function copyNumberString(num) {
    ...
    localStorage.setItem('ht_copied_highscale', JSON.stringify(copiedNumbersMap));
    renderRangeNumbers();
    showToast("Copied");
}
...
function resetRangeCopied() {
    if (confirm("Reset copied numbers status?")) {
        copiedNumbersMap = {};
        localStorage.removeItem('ht_copied_highscale');
        renderRangeNumbers();
        showToast("Reset Done");
    }
}

// 7. Live Inbox Feed from Lamix
function renderLiveInbox(messages = liveMessagesList) {
    const container = document.getElementById('inboxFeedContainer');

    if (messages.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:35px; color:#8c9bb5;">No incoming messages received in Lamix yet.</div>`;
        return;
    }

    container.innerHTML = messages.map(m => {
        const sender = m.sender || m.from || m.phone || 'Live Sender';
...
checkAppSession();
renderTop10();
setInterval(loadLivePortalData, 7000); // Poll live API every 7 seconds
