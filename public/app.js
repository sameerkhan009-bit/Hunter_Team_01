...
let copiedNumbersMap = JSON.parse(localStorage.getItem('ht_copied_nums_final')) || {};
...
function copyNumberString(num) {
    ...
    localStorage.setItem('ht_copied_nums_final', JSON.stringify(copiedNumbersMap));
    renderRangeNumbers();
    showToast("Copied");
}
...
function resetRangeCopied() {
    if (confirm("Reset copied numbers status?")) {
        copiedNumbersMap = {};
        localStorage.removeItem('ht_copied_nums_final');
        renderRangeNumbers();
...
checkAppSession();
renderTop10();
setInterval(fetchLiveLamixData, 8000); // Live poll Lamix API
